...
router.get('/info', async (req, res, next) => {
  ...
  sqlSyntax = `
    SELECT id, apply_id, name, email, mobile, wechat, grade, content, remarks, create_time, editor_time, editor_id, status
    FROM calvin.apply_tbl
    WHERE id = ?
  `
...
router.post('/create', async (req, res, next) => {
  let sqlSyntax = ``
  let results
  const { apply_id, name, email, mobile, wechat, grade, content } = req.body

  if (!apply_id || !name || !email || !mobile || !wechat || !grade || !content) {
  	res.send({
      code: 10000,
      message: '参数缺失'
    })
  ...
  sqlSyntax = `
    SELECT apply_id, name, email, mobile, wechat, grade, content
    FROM calvin.apply_tbl
    WHERE  apply_id = ? AND name = ? AND email = ? AND mobile = ? AND wechat = ? AND grade = ? AND  content = ?
  `

  results = await db.exec(sqlSyntax, [ apply_id, name, email, mobile, wechat, grade, content ])
  ...
  sqlSyntax = `
  	INSERT INTO calvin.apply_tbl (apply_id, name, email, mobile, wechat, grade, content, create_time)
...
router.get('/list', async (req, res, next) => {
  const { apply_id, name, email, mobile, wechat, grade, content, status, create_timerange, pageNum = 1, pageSize = 10 } = req.query
  const sqlQueue = []
  let sqlSyntax = ``

  sqlSyntax = `
    SELECT SQL_CALC_FOUND_ROWS id, apply_id, name, email, mobile, wechat, grade, content, status, create_time, editor_time, editor_id
    FROM calvin.apply_tbl
  `
